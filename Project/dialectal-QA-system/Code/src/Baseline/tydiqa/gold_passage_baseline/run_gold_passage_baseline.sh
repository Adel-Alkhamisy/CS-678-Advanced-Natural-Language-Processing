...
MBERT_MODEL_DIR="${TYDIQA_REPO_DIR}/../multi_cased_L-12_H-768_A-12"
...
TYDIQA_GOLDP_DIR="${TYDIQA_REPO_DIR}/../data"
VERSION=""
...
python $BERT_CODE_DIR/run_squad.py \
  ...
  --do_train=True \
  --train_file="${TYDIQA_GOLDP_DIR}/dialqa-train${VERSION}.json" \
  --do_predict=True \
  --predict_file="${TYDIQA_GOLDP_DIR}/dialqa-dev-aug${VERSION}.json" \
  --train_batch_size=12 \
  --learning_rate=3e-5 \
